            ...
            case "$2" in
                system)
                    ...
                    if command -v mkfs.ext4 >/dev/null 2>&1; then
                        ...
                        mke2fs -t ext4 -F "$OUT/rootfs.img"
                    else
                        logit "Neither mkfs.ext4 nor mke2fs is available – please install e2fsprogs"
                        exit 1
                    fi
                    ...
                    if command -v dumpe2fs >/dev/null 2>&1; then
                        ...
                        logit "dumpe2fs not found – skipping orphan_file check"
                    fi
                ;;
...
# Detach loop device if still present and clean up mountpoint
if [ -n "$LOOPDEV" ] && [ -e "$LOOPDEV" ]; then
    sudo losetup -d "$LOOPDEV" || true
fi
# Unmount the system mountpoint if it is still mounted (lazy unmount)
if mount | grep -q " $SYSTEM_MOUNTPOINT "; then
    sudo umount -l "$SYSTEM_MOUNTPOINT" || true
fi
# Remove the temporary system mount directory
sudo rm -rf "$SYSTEM_MOUNTPOINT" || true
